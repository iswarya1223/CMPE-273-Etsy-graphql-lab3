...
const OrderResolvers={
    Query:
    {
        async getorderdetails({ email, resultsperpage,currentPage}){
            try {
            const ordersCount = await Order.countDocuments();
            let results = await Order.find({ email: email}).limit(parseInt(resultsperpage)*1).skip(parseInt(resultsperpage)*(currentPage-1))
    ...
    Mutation:{
        async addorder({ email,totalprice }){
